package common;

/**
 * Contains various String constants used to convey info through Message objects.
 * @author etudiant
 *
 */
public class MessageInfoStrings {
	public static final String REQUEST_IP_TARGET_PORT = "targetPort";
	public static final String REQUEST_IP_TARGET_IP = "targetIp";
	public static final String GENERIC_PORT = "port";
	public static final String GENERIC_PASSWORD = "pass";
	public static final String GENERIC_LOGIN = "login";
	public static final String REQUEST_LIST_CLIENT_LOGINS = "clientLogins";

	/**
	 * Do not instantiate this class.
	 */
	private MessageInfoStrings() {}
}
